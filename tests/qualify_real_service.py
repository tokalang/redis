#!/usr/bin/env python3
"""Qualify official/redis against real loopback Redis services.
...
def resolve_toolchain(
    env: dict[str, str], explicit_tokac: Path | None = None
) -> tuple[Path, Path]:
    if explicit_tokac is not None:
        tokac = explicit_tokac.expanduser().resolve()
        if "TOKA_LIB" in env:
            library = Path(env["TOKA_LIB"]).expanduser().resolve()
        elif "TOKA_ROOT" in env:
            library = Path(env["TOKA_ROOT"]).expanduser().resolve() / "lib"
        else:
            library = tokac.parents[1] / "lib"
    elif "TOKA_ROOT" in env:
        root = Path(env["TOKA_ROOT"]).expanduser().resolve()
        tokac = root / "build" / "bin" / "tokac"
        library = root / "lib"
    elif "TOKAC" in env and "TOKA_LIB" in env:
        tokac = Path(env["TOKAC"]).expanduser().resolve()
        library = Path(env["TOKA_LIB"]).expanduser().resolve()
    else:
        tokac = PACKAGE.parents[0] / "toka" / "build" / "bin" / "tokac"
        library = PACKAGE.parents[0] / "toka" / "lib"
    return tokac, library


def runner_prerequisite(tokac: Path, toka_lib: Path) -> tuple[bool, str]:
    if not tokac.is_file() or not os.access(tokac, os.X_OK):
        return False, f"Toka compiler is not executable: {tokac}"
    if not toka_lib.is_dir():
...
def compile_fixture(tokac: Path, toka_lib: Path, source: str, output: Path) -> None:
    env = dict(os.environ)
...
def published_port(container: str, container_port: int) -> int:
    published = command(
        ["docker", "port", container, f"{container_port}/tcp"], timeout=20
    )
...
def image_digest(image: str) -> str:
    try:
        ...
        try:
            return command(
                ["docker", "inspect", "--format", "{{.Id}}", image], timeout=15
            )
        except Exception:
            return "unknown"
...
def qualify_redis(
    ...
    try:
        ...
        digest_text = image_digest(image)
        return {
            "image": image,
            "image_digest": digest_text,
            "server": version_text,
            "transport": "tls-private-ca" if tls else "tcp-auth",
        }
    except Exception as error:
        raise QualificationFailure(
            f"Redis {version} ({'TLS' if tls else 'TCP'}) failed: {error}\n{service_log(name)}"
        ) from error
    finally:
...
def main() -> int:
    ...
    parser.add_argument("--tokac", type=Path, help="path to tokac compiler executable")
    ...
    tokac, toka_lib = resolve_toolchain(host_env, args.tokac)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="toka-redis-real-") as temporary:
            ...
            compile_fixture(tokac, toka_lib, "real_service_v1.tk", redis_fixture)
            compile_fixture(tokac, toka_lib, "clone_ownership_v1.tk", redis_clone_fixture)
